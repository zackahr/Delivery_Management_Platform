...
const TableCommands2: React.FC = () => {
  ...
  const handleModify = async () => {
    if (selectedCommand) {
      try {
        ...
        const response = await axios.patch(`http://64.226.75.205:3000/commands/${selectedCommand._id}`, updateData);
  ...
  const handleDelete = async (command: Command) => {
    try {
      await axios.delete(`http://64.226.75.205:3000/commands/${command._id}`);
      // Assuming success, you may want to update state or handle success feedback
